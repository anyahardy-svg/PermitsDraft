...
const PermitManagementApp = () => {
  ...
  const [users, setUsers] = useState([
    { id: 'user-001', name: 'John Smith', email: 'john.smith@company.com', sites: ['Amisfield Quarry', 'Belmont Quarry'], company: 'ABC Contractors' },
    { id: 'user-002', name: 'Jane Doe', email: 'jane.doe@company.com', sites: ['Wheatsheaf Quarry'], company: 'ABC Contractors' },
    { id: 'user-003', name: 'Bob Wilson', email: 'bob.wilson@company.com', sites: ['Otaki Quarry', 'Petone Quarry'], company: 'XYZ Services' }
  ]);

  // Contractors state - stores contractor information
  const [contractors, setContractors] = useState([
    { id: 'contractor-001', name: 'ABC Contractors', email: 'info@abc-contractors.com', services: ['Hot Work', 'Electrical'], company: 'ABC Contractors Ltd' },
    { id: 'contractor-002', name: 'XYZ Services', email: 'contact@xyz-services.com', services: ['Confined Space', 'Working at Height'], company: 'XYZ Services Inc' }
  ]);
  ...
  const [currentUser, setCurrentUser] = useState({ id: '', name: '', email: '', sites: [], company: '' });
  const [selectedContractor, setSelectedContractor] = useState(null);
  const [editingContractor, setEditingContractor] = useState(false);
  const [currentContractor, setCurrentContractor] = useState({ id: '', name: '', email: '', services: [], company: '' });
...
function ReviewPermitScreen({ permit, setPermits, setCurrentScreen, permits, styles }) {
  ...
  const renderManageUsers = () => {
    const handleAddUser = () => {
      ...
      setCurrentUser({ id: '', name: '', email: '', sites: [], company: '' });
    ...
    const handleDeleteUser = (id) => {
      Alert.alert('Delete User', 'Are you sure?', [
        ...
        { text: 'Delete', onPress: () => setUsers(users.filter(u => u.id !== id)), style: 'destructive' }
      ]);
    };
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ flexGrow: 1, padding: 16 }}>
          ...
          <View style={styles.section}>
            <View style={styles.sectionContent}>
              ...
              {editingUser && (
                <TouchableOpacity style={[styles.addButton, { backgroundColor: '#EF4444' }]} onPress={() => { setEditingUser(false); setCurrentUser({ id: '', name: '', email: '', sites: [], company: '' }); setSelectedUser(null); }}>
  ...
  const renderManageContractors = () => {
    const handleAddContractor = () => {
      ...
      if (editingContractor) {
        setContractors(contractors.map(c => c.id === currentContractor.id ? currentContractor : c));
        ...
        setContractors([...contractors, { ...currentContractor, id: newId }]);
        Alert.alert('Contractor Added', 'New contractor has been added successfully.');
      }
      setCurrentContractor({ id: '', name: '', email: '', services: [], company: '' });
    ...
    const handleDeleteContractor = (id) => {
      Alert.alert('Delete Contractor', 'Are you sure?', [
        { text: 'Cancel', style: 'cancel' },
        { text: 'Delete', onPress: () => setContractors(contractors.filter(c => c.id !== id)), style: 'destructive' }
      ]);
    };
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ flexGrow: 1, padding: 16 }}>
          ...
          <View style={styles.section}>
            <View style={styles.sectionContent}>
              ...
              {editingContractor && (
                <TouchableOpacity style={[styles.addButton, { backgroundColor: '#EF4444' }]} onPress={() => { setEditingContractor(false); setCurrentContractor({ id: '', name: '', email: '', services: [], company: '' }); setSelectedContractor(null); }}>
                  <Text style={styles.addButtonText}>Cancel</Text>
                </TouchableOpacity>
              )}
            </View>
          ...
          <View style={{ marginTop: 24 }}>
            ...
            <Text style={{ color: '#6B7280', marginTop: 4, marginBottom: 12 }}>Total: {contractors.length} contractors</Text>
            {contractors.length === 0 ? (
              <Text style={{ textAlign: 'center', marginTop: 20, color: '#9CA3AF' }}>No contractors yet. Add one using the form above.</Text>
            ) : (
              contractors.map((contractor, index) => (
                <View key={contractor.id} style={[styles.permitListCard, { marginBottom: 12 }]}>
                  <Text style={styles.permitId}>{index + 1}. {contractor.name}</Text>
                  <Text style={styles.detailText}>{contractor.email}</Text>
                  <Text style={styles.detailText}>{contractor.company}</Text>
                  <Text style={[styles.detailText, { marginTop: 8 }]}>Services: {contractor.services.length > 0 ? contractor.services.join(', ') : 'None'}</Text>
                  <View style={{ flexDirection: 'row', marginTop: 12, gap: 8 }}>
                    <TouchableOpacity style={[styles.addButton, { flex: 0.45 }]} onPress={() => { setSelectedContractor(contractor); setEditingContractor(true); setCurrentContractor(contractor); }}>
                      <Text style={styles.addButtonText}>Edit</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={[styles.addButton, { flex: 0.45, backgroundColor: '#EF4444' }]} onPress={() => handleDeleteContractor(contractor.id)}>
                      <Text style={styles.addButtonText}>Delete</Text>
                    </TouchableOpacity>
                  </View>
                </View>
    ...
    return (
      <ScrollView style={{ flex: 1, backgroundColor: '#F9FAFB' }} contentContainerStyle={{ padding: 16 }}>
        ...
        <View style={styles.sectionContent}>
          ...
          <TextInput style={styles.input} value={editData.endDate || ''} onChangeText={text => setEditData({ ...editData, endDate: text })} placeholder="End Date" />
          <TextInput style={styles.input} value={editData.endTime || ''} onChangeText={text => setEditData({ ...editData, endTime: text })} placeholder="End Time" />
          {/* Specialized Permits - use the same questionnaire UI with toggles */}
          {editData.specializedPermits && permitQuestionnaires && (
            <View style={{ marginTop: 12 }}>
              <Text style={styles.label}>Specialized Permits:</Text>
              {Object.entries(editData.specializedPermits).map(([key, val]) => (
          ...
          {/* Single Hazards - always show all, allow editing with toggle */}
          {editData.singleHazards && (
            <View style={{ marginTop: 12 }}>
              <Text style={styles.label}>Single Hazards:</Text>
              {Object.entries(editData.singleHazards).map(([key, val]) => (
