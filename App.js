...
const PermitManagementApp = ({ initialSiteId, onBackToKiosk, initialAdminRoute, initialCompanyAccreditationId, initialContractorAdminTab }) => {
  ...
  // Save current JSEA as a template
  const handleSaveJseaAsTemplate = async () => {
  ...
  const [currentPermitIssuer, setCurrentPermitIssuer] = useState({ id: '', name: '', email: '', sites: [], businessUnitIds: [], isAdmin: false, permittedServiceIds: [] });
  const [businessUnits, setBusinessUnits] = useState([]);
  const [servicesForBusinessUnits, setServicesForBusinessUnits] = useState([]);
  ...
  const handleVerifyPermit = async (permit, verifierName) => {
    try {
      ...
      const { data, error } = await supabaseClient
        ...
        .update({
          ...
          verified_by: verifierName || 'Unknown'
        })
        .eq('id', permit.id);
      ...
      setPermits(prev => prev.map(p => 
        p.id === permit.id 
          ? { ...p, last_verified_at: now, verified_by: verifierName || 'Unknown' }
          : p
      ));
      
      Alert.alert('Success', `Permit verified by ${verifierName}`);
    } catch (err) {
  ...
  const renderNewPermitForm = () => {
    ...
    return (
      <View style={{ flex: 1 }}>
        ...
        >
                            ...
                            <View style={{ position: 'relative', marginBottom: 12, overflow: 'visible', zIndex: 9999 }}>
                              <TextInput 
                                ...
                                onChangeText={text => {
                                  ...
                                  // Filter contractors by site (if selected) or show all matching contractors
                                ...
                                onFocus={() => {
                                  if ((formData.specializedPermits[permit.key].questionnaire?.hw_safety_watch?.text || '').trim().length > 0) {
                                    let siteContractors = contractors;
                                    if (formData.site) {
                                      const siteId = siteNameToIdMap[formData.site];
                                      siteContractors = contractors.filter(c => 
                          ...
                          <View style={{ marginTop: 12, marginBottom: 12, backgroundColor: '#FEF3C7', padding: 12, borderRadius: 8, borderLeftWidth: 4, borderLeftColor: '#F59E0B', overflow: 'visible', zIndex: 100, position: 'relative' }}>
                            ...
                            <View style={{ position: 'relative', marginBottom: 12, overflow: 'visible', zIndex: 9999 }}>
                              <TextInput 
                                ...
                                onFocus={() => {
                                  if ((formData.specializedPermits[permit.key].questionnaire?.safety_watch_name?.text || '').trim().length > 0) {
                                    let siteContractors = contractors;
                                    if (formData.site) {
                                      const siteId = siteNameToIdMap[formData.site];
                                      siteContractors = contractors.filter(c => 
          ...
          <View style={styles.section}>
            <TouchableOpacity style={styles.sectionHeader} onPress={() => toggleSection('jsea')}>
              <Text style={styles.sectionTitle}>JSEA ({formData.jseas?.length || 0})</Text>
              ...
              <View style={styles.sectionContent}>
                ...
                <View style={{ marginBottom: 16 }}>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                    <Text style={styles.label}>Added JSEAs</Text>
                    <TouchableOpacity 
                      style={[styles.addButton, { paddingVertical: 8, paddingHorizontal: 12 }]}
                      onPress={() => openJseaEditor(null)}
                    >
                      <Text style={styles.addButtonText}>+ Add JSEA</Text>
                    </TouchableOpacity>
                  </View>
                  
                  {(!formData.jseas || formData.jseas.length === 0) ? (
                    <View style={{ padding: 12, backgroundColor: '#F3F4F6', borderRadius: 6 }}>
                      <Text style={{ color: '#6B7280', textAlign: 'center' }}>No JSEAs added yet</Text>
                    </View>
                  ) : (
                    <View style={{ gap: 10 }}>
                      {formData.jseas.map((jsea, idx) => (
                        <View key={jsea.id || idx} style={{ padding: 12, backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 6 }}>
                          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                            <Text style={{ fontSize: 14, fontWeight: '600', flex: 1, color: '#374151' }}>
                              {jsea.title || `JSEA ${idx + 1}`}
                            </Text>
                            <View style={{ flexDirection: 'row', gap: 8 }}>
                              <TouchableOpacity 
                                style={{ padding: 8, backgroundColor: '#3B82F6', borderRadius: 4 }}
                                onPress={() => openJseaEditor(idx)}
                              >
                                <Text style={{ color: 'white', fontSize: 12, fontWeight: '600' }}>Edit</Text>
                              </TouchableOpacity>
                              <TouchableOpacity 
                                style={{ padding: 8, backgroundColor: '#EF4444', borderRadius: 4 }}
                                onPress={() => deleteJsea(idx)}
                              >
                          ...
                          <View style={{ gap: 8 }}>
                            <View>
                              <Text style={{ fontSize: 11, color: '#6B7280', fontWeight: '500', marginBottom: 4 }}>Task Steps ({jsea.taskSteps?.length || 0})</Text>
                              {jsea.taskSteps && jsea.taskSteps.length > 0 ? (
                                <View style={{ paddingLeft: 8 }}>
                                  {jsea.taskSteps.slice(0, 3).map((step, stepIdx) => (
                                    <Text key={stepIdx} style={{ fontSize: 10, color: '#4B5563', marginBottom: 2 }}>
                                      • {step.description.substring(0, 40)}{step.description.length > 40 ? '...' : ''}
                                    </Text>
                                  ))}
                                  {jsea.taskSteps.length > 3 && (
                                    <Text style={{ fontSize: 10, color: '#9CA3AF', fontStyle: 'italic' }}>
                                      +{jsea.taskSteps.length - 3} more steps
                                    </Text>
                                  )}
                                </View>
                              ) : (
                                <Text style={{ fontSize: 10, color: '#9CA3AF' }}>No task steps</Text>
                              )}
                            </View>
                            
                            <View>
                              <Text style={{ fontSize: 11, color: '#6B7280', fontWeight: '500', marginBottom: 4 }}>Risk Rating</Text>
                              <View style={{
                                display: 'flex',
                                backgroundColor: jsea.overallRiskRating ? getRiskColor(jsea.overallRiskRating) : '#D1D5DB',
                                paddingHorizontal: 8,
                                paddingVertical: 4,
                                borderRadius: 4,
                                ...
                                <Text style={{ color: 'white', fontWeight: '600', fontSize: 10 }}>
                                  {jsea.overallRiskRating?.toUpperCase() || 'NOT SET'}
                                </Text>
                              </View>
                            </View>
                          </View>
                  ...
                  <View style={{ marginTop: 16, paddingTop: 16, borderTopWidth: 1, borderTopColor: '#E5E7EB' }}>
                    ...
                    <View style={{ marginBottom: 16 }}>
                      ...
                      <TextInput
                        ...
                        placeholder="e.g., Screen Replacement, Ladder Work..."
                      />
                    </View>
                    ...
                    <View style={{ marginBottom: 12 }}>
                      ...
                      <View style={{ flexDirection: 'row', gap: 8, marginBottom: 12 }}>
                        <TouchableOpacity style={[styles.addButton, { flex: 1 }]} onPress={handleEditJseaDraft}>
                          <Text style={styles.addButtonText}>Edit Task Steps</Text>
                        </TouchableOpacity>
                        <TouchableOpacity 
              ...
              <View style={[styles.sectionContent, { backgroundColor: '#FEF3C7', borderLeftWidth: 4, borderLeftColor: '#F59E0B' }]}>
                ...
                {(() => {
                  ...
                  if (formData.specializedPermits.hotWork?.required) {
                    const hwSafetyWatchObj = formData.specializedPermits.hotWork.questionnaire?.hw_safety_watch;
                    const hwSafetyWatch = typeof hwSafetyWatchObj === 'string' ? hwSafetyWatchObj : hwSafetyWatchObj?.text;
                    const hwSafetyPeriodObj = formData.specializedPermits.hotWork.questionnaire?.hw_safety_period;
                  ...
                  if (formData.specializedPermits.confinedSpace?.required) {
                    ...
                    const csSafetyWatchNameObj = formData.specializedPermits.confinedSpace.questionnaire?.safety_watch_name;
                    const csSafetyWatchName = typeof csSafetyWatchNameObj === 'string' ? csSafetyWatchNameObj : csSafetyWatchNameObj?.text;
        ...
        {/* JSEA Save as Template Modal */}
        <Modal 
          visible={showJseaSaveTemplate} 
          animationType="slide"
          onRequestClose={() => setShowJseaSaveTemplate(false)}
          transparent
        >
          <View style={{
            flex: 1,
              ...
              <View style={{ flexDirection: 'row', gap: 8 }}>
                ...
                <TouchableOpacity
                  style={{
                    ...
                    borderRadius: 8,
                    alignItems: 'center',
                    opacity: loadingJseaTemplates ? 0.6 : 1
                  }}
                  onPress={handleSaveJseaAsTemplate}
                  disabled={loadingJseaTemplates}
                >
                  <Text style={{
                    fontSize: 14,
                    fontWeight: '600',
        ...
        <View style={{ marginTop: 12, padding: 10, backgroundColor: '#FEF3C7', borderRadius: 6, borderLeftWidth: 3, borderLeftColor: '#F59E0B' }}>
          ...
          {(() => {
            ...
            if (item.specializedPermits?.hotWork?.required) {
              // Check both possible structures: { text: "..." } and direct string value
              const hwSafetyWatchObj = item.specializedPermits.hotWork.questionnaire?.hw_safety_watch;
              const hwSafetyWatch = typeof hwSafetyWatchObj === 'string' ? hwSafetyWatchObj : hwSafetyWatchObj?.text;
              const hwSafetyPeriodObj = item.specializedPermits.hotWork.questionnaire?.hw_safety_period;
              const hwSafetyPeriod = typeof hwSafetyPeriodObj === 'string' ? hwSafetyPeriodObj : hwSafetyPeriodObj?.answer;
              if (hwSafetyWatch) {
                safetyWatches.push({
                  permit: 'Hot Work',
            ...
            if (item.specializedPermits?.confinedSpace?.required) {
              ...
              const csSafetyWatchNameObj = item.specializedPermits.confinedSpace.questionnaire?.safety_watch_name;
              const csSafetyWatchName = typeof csSafetyWatchNameObj === 'string' ? csSafetyWatchNameObj : csSafetyWatchNameObj?.text;
              if (csSafetyWatch === 'yes' || csSafetyWatchName) {
                safetyWatches.push({
                  permit: 'Confined Space',
          ...
          <TouchableOpacity style={[styles.submitButton, { backgroundColor: '#EF4444', marginLeft: 12 }]} onPress={async () => {
            ...
            if (window.confirm('Delete Draft Permit?\n\nAre you sure you want to reject this draft permit? This will permanently delete it and cannot be undone.')) {
              try {
                ...
                const updated = permits.map(p => p.id === editData.id ? { ...editData, status: 'rejected' } : p);
                setPermits(updated);
                setCurrentScreen('dashboard');
  ...
  const renderManagePermitIssuers = () => {
    const handleAddPermitIssuer = async () => {
      if (!currentPermitIssuer.name || !currentPermitIssuer.email || currentPermitIssuer.businessUnitIds.length === 0) {
        Alert.alert('Missing Info', 'Please fill in Name, Email, and select at least one Business Unit.');
        return;
      }
      try {
        ...
        if (editingPermitIssuer) {
          await updatePermitIssuer(currentPermitIssuer.id, {
            ...
            businessUnitIds: currentPermitIssuer.businessUnitIds,
        ...
        setCurrentPermitIssuer({ id: '', name: '', email: '', sites: [], businessUnitIds: [], isAdmin: false, permittedServiceIds: [] });
        ...
        <ScrollView ref={permitIssuersScrollRef} style={{ flex: 1 }} contentContainerStyle={{ flexGrow: 1, padding: 16 }}>
          ...
          <View style={styles.section}>
            <View style={styles.sectionContent}>
              ...
              <TextInput style={styles.input} value={currentPermitIssuer.name} onChangeText={text => setCurrentPermitIssuer({ ...currentPermitIssuer, name: text })} placeholder="Enter issuer name" />
              
              <Text style={styles.label}>Email Address *</Text>
              <TextInput style={styles.input} value={currentPermitIssuer.email} onChangeText={text => setCurrentPermitIssuer({ ...currentPermitIssuer, email: text })} placeholder="email@company.com" keyboardType="email-address" />
              ...
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginBottom: 12 }}>
                {businessUnits.map(unit => {
                  const isSelected = currentPermitIssuer.businessUnitIds.includes(unit.id);
                  return (
                    <TouchableOpacity
                      key={unit.id}
                      style={[
                      ...
                      onPress={async () => {
                        ...
                        if (isSelected) {
                          updatedBusinessUnitIds = currentPermitIssuer.businessUnitIds.filter(id => id !== unit.id);
                        } else {
                          updatedBusinessUnitIds = [...currentPermitIssuer.businessUnitIds, unit.id];
                        }
                        ...
                        if (updatedBusinessUnitIds.length > 0) {
                          ...
                          setServicesForBusinessUnits([]);
                          setSitesForBusinessUnits([]);
                        }
                      }}
                    >
              ...
              {currentPermitIssuer.businessUnitIds.length > 0 && servicesForBusinessUnits.length > 0 && (
                <>
              ...
              <Text style={{ color: '#6B7280', marginBottom: 8 }}>
                {currentPermitIssuer.businessUnitIds.length > 0 
              ...
              {currentPermitIssuer.businessUnitIds.length === 0 ? (
                ...
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginBottom: 12 }}>
                  {sitesForBusinessUnits.length > 0 ? sitesForBusinessUnits.map(siteObj => {
                    const siteName = typeof siteObj === 'string' ? siteObj : siteObj.name;
                    const isSelected = currentPermitIssuer.sites.includes(siteName) || 
                      (currentPermitIssuer.site_ids && currentPermitIssuer.site_ids.some(id => siteIdToNameMap[id] === siteName));
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          if (currentPermitIssuer.sites.includes(siteName)) {
                            setCurrentPermitIssuer({ ...currentPermitIssuer, sites: currentPermitIssuer.sites.filter(s => s !== siteName) });
                          } else {
                            setCurrentPermitIssuer({ ...currentPermitIssuer, sites: [...currentPermitIssuer.sites, siteName] });
                          }
                        }}
                      >
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16, padding: 12, backgroundColor: '#F3F4F6', borderRadius: 8 }}>
                <TouchableOpacity
                  style={{
                    ...
                    borderRadius: 4,
                    borderWidth: 2,
                    borderColor: currentPermitIssuer.isAdmin ? '#2563EB' : '#D1D5DB',
                    backgroundColor: currentPermitIssuer.isAdmin ? '#2563EB' : 'white',
                    justifyContent: 'center',
                    alignItems: 'center',
                    marginRight: 12
                  }}
                  onPress={() => setCurrentPermitIssuer({ ...currentPermitIssuer, isAdmin: !currentPermitIssuer.isAdmin })}
                >
              ...
              {editingPermitIssuer && (
                <TouchableOpacity style={[styles.addButton, { backgroundColor: '#EF4444' }]} onPress={() => { setEditingPermitIssuer(false); setCurrentPermitIssuer({ id: '', name: '', email: '', sites: [], businessUnitIds: [], isAdmin: false, permittedServiceIds: [] }); setSelectedPermitIssuer(null); }}>
                  <Text style={styles.addButtonText}>Cancel</Text>
                </TouchableOpacity>
              )}
            </View>
  ...
  const renderManageCompanies = () => {
    ...
    return (
      <View style={{ flex: 1, backgroundColor: '#F9FAFB' }}>
        ...
        {showTrainingRecordsModal && selectedCompanyForTrainingRecords && (
          <Modal
            visible={showTrainingRecordsModal}
            transparent={false}
            animationType="slide"
            onRequestClose={() => setShowTrainingRecordsModal(false)}
          >
            <View style={{ flex: 1, backgroundColor: '#F9FAFB' }}>
              {/* Header */}
              <View style={{
 ...
 0             {importStatus === 'importing' && '⏳ ' }
            ...
            <ScrollView style={{ flex: 1, borderRightWidth: 1, borderRightColor: '#E5E7EB', padding: 16 }}>
              ...
              {visitorInductionContent.map((line, idx) => (
                <View key={idx} style={{ marginBottom: 12, backgroundColor: 'white', borderRadius: 8, borderWidth: 1, borderColor: '#D1D5DB', padding: 12 }}>
                  {/* Format Buttons - Always visible, single tap to change */}
                  <View style={{ flexDirection: 'row', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>
                    {[
                      ...
                      <TouchableOpacity
                        ...
                        style={{
                          ...
                          borderRadius: 4,
                          backgroundColor: line.type === option.type ? '#3B82F6' : '#E5E7EB',
                          borderWidth: line.type === option.type ? 2 : 1,
                          borderColor: line.type === option.type ? '#1E40AF' : '#D1D5DB',
                        }}
                      >
                        <Text style={{
                          fontSize: 11,
                    ...
                    {/* Delete Button */}
                    <TouchableOpacity
                      style={{ marginLeft: 'auto', paddingVertical: 6, paddingHorizontal: 10, borderRadius: 4, backgroundColor: '#FEE2E2' }}
                      onPress={() => {
  ...
  const EditableApprovalPermitScreen = ({ permit, setPermits, setCurrentScreen, permits, styles, handlePrintPermit, sites, users, contractors, servicesFromDb, siteNameToIdMap, siteIdToNameMap, permitQuestionnaires, specializedPermitTypes, singleHazardTypes, getRiskColor, isolationRegisters, setShowJseaEditor, setSelectedBuForLoader, setShowJseaTemplateLoader, setShowJseaSaveTemplate, loadJseaTemplatesForLoader, businessUnitId, setShowRiskMatrix, setSelectedLikelihood, setSelectedSeverity, setRiskMatrixContext }) => {
    ...
    const [filteredHwSafetyWatchContractorsDraft, setFilteredHwSafetyWatchContractorsDraft] = React.useState({});
    const [showCsSafetyWatchDropdownDraft, setShowCsSafetyWatchDropdownDraft] = React.useState({});
    const [filteredCsSafetyWatchContractorsDraft, setFilteredCsSafetyWatchContractorsDraft] = React.useState({});
                                ...
                                <TextInput 
                                  ...
                                  onChangeText={text => {
                                    ...
                                    if (text.trim().length > 0 && contractors && contractors.length > 0) {
                                      ...
                                      setFilteredHwSafetyWatchContractorsDraft(prev => ({ ...prev, [key]: filtered }));
                                      setShowHwSafetyWatchDropdownDraft(prev => ({ ...prev, [key]: filtered.length > 0 }));
                                    } else {
                                      setShowHwSafetyWatchDropdownDraft(prev => ({ ...prev, [key]: false }));
                                      setFilteredHwSafetyWatchContractorsDraft(prev => ({ ...prev, [key]: [] }));
                                    }
                                  }}
                                  onFocus={() => {
                                  ...
                                  onBlur={() => {
                                    setTimeout(() => setShowHwSafetyWatchDropdownDraft(prev => ({ ...prev, [key]: false })), 500);
                                  }}
                                  placeholder="Start typing person name (contractor or employee)..."
                                />
                                {!editData.site && (
                                ...
                                <TextInput 
                                  ...
                                  onChangeText={text => {
                                    ...
                                    if (text.trim().length > 0 && contractors && contractors.length > 0) {
                                      ...
                                      setFilteredCsSafetyWatchContractorsDraft(prev => ({ ...prev, [key]: filtered }));
                                      setShowCsSafetyWatchDropdownDraft(prev => ({ ...prev, [key]: filtered.length > 0 }));
                                    } else {
                                      setShowCsSafetyWatchDropdownDraft(prev => ({ ...prev, [key]: false }));
                                      setFilteredCsSafetyWatchContractorsDraft(prev => ({ ...prev, [key]: [] }));
                                    }
                                  }}
                                  onFocus={() => {
                                  ...
                                  onBlur={() => {
                                    setTimeout(() => setShowCsSafetyWatchDropdownDraft(prev => ({ ...prev, [key]: false })), 500);
                                  }}
                                  placeholder="Start typing person name (contractor or employee)..."
                                />
                                {!editData.site && (
            ...
            {expandedSections.isolations && (
              <View style={styles.sectionContent}>
                ...
                <View style={{ marginBottom: 12, padding: 8, backgroundColor: '#F0F9FF', borderRadius: 6, borderLeftWidth: 3, borderLeftColor: '#0EA5E9' }}>
                  ...
                  <TouchableOpacity 
                    style={[styles.input, { backgroundColor: 'white', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }]}
                    onPress={() => setIsolationDropdownOpen(!isolationDropdownOpen)}
                  >
                    <Text style={{ color: selectedIsolationId ? '#374151' : '#9CA3AF' }}>
                      {selectedIsolationId 
                        ? isolationRegisters.find(r => r.id === selectedIsolationId)?.main_lockout_item 
                        : 'Select main lockout item...'}
                    </Text>
                    <Text style={{ fontSize: 16 }}>{isolationDropdownOpen ? '▲' : '▼'}</Text>
                  </TouchableOpacity>
                  
                  {isolationDropdownOpen && (
                    <View style={{ 
                      backgroundColor: 'white', 
                      marginTop: 4, 
                      borderWidth: 1, 
                      ...
                      {isolationRegisters
                        ...
                        .map(isolation => (
                          <TouchableOpacity
                            ...
                            style={{ padding: 10, borderBottomWidth: 1, borderBottomColor: '#F3F4F6' }}
                            onPress={() => {
                              setSelectedIsolationId(isolation.id);
                              setIsolationDropdownOpen(false);
                              // Add the selected isolation to the list
                              const newIsolation = {
                                main_lockout_item: isolation.main_lockout_item,
                                linked_items: [isolation.linked_item_1, isolation.linked_item_2, isolation.linked_item_3, isolation.linked_item_4, isolation.linked_item_5, isolation.linked_item_6, isolation.linked_item_7, isolation.linked_item_8, isolation.linked_item_9, isolation.linked_item_10].filter(Boolean),
                              ...
                              setEditData(prev => ({ ...prev, isolations: [...(prev.isolations || []), newIsolation] }));
                            }}
                          >
                            <Text style={{ color: '#374151', fontWeight: '500' }}>{isolation.main_lockout_item}</Text>
                          </TouchableOpacity>
                  ...
                  {selectedIsolationId && (
                    <View style={{ marginTop: 12, padding: 8, backgroundColor: '#E0F2FE', borderRadius: 4 }}>
                      {(() => {
                        const selected = isolationRegisters.find(r => r.id === selectedIsolationId);
                        if (!selected) return null;
                        const linkedItems = [selected.linked_item_1, selected.linked_item_2, selected.linked_item_3, selected.linked_item_4, selected.linked_item_5, selected.linked_item_6, selected.linked_item_7, selected.linked_item_8, selected.linked_item_9, selected.linked_item_10].filter(Boolean);
                        return (
                          <View>
  ...
  const EditInspectionPermitScreen = ({ permit, setPermits, setCurrentScreen, permits, styles, sites, users, siteNameToIdMap, siteIdToNameMap, permitQuestionnaires, specializedPermitTypes, singleHazardTypes, getRiskColor, isolationRegisters, businessUnitId, contractors, servicesFromDb, initialSiteId }) => {
    ...
    // Isolated by contractor dropdown states
    const [showIsolatedByDropdownActive, setShowIsolatedByDropdownActive] = React.useState({});
    const [filteredIsolatedByContractorsActive, setFilteredIsolatedByContractorsActive] = React.useState({});
